...
public class AccountService {
    ...
    public GetAccountsResponseDto getAccounts(Long userId, List<GetAccountsRequestDto.BankDto> bankDtos) {
        ...
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "[AccountService]"));
        String userKey = user.getUserKey();

        // SSAFY 금융 API >>>>> 2.4.4 계좌 목록 조회
        // 요청보낼 url
        String url2 = ssafyBaseUrl + "/ssafy/api/v1/edu/demandDeposit/inquireDemandDepositAccountList";
        ...
        for (AccountDto accountDto : filteredAccounts) {
            ...
            Bank bank = bankRepository.findByCode(accountDto.getBankCode())
                    .orElseThrow(() -> new AppException(ErrorCode.BANK_NOT_FOUND, "AccountService - 000"));

            GetAccountsResponseDto.AccountResponseDto accountResponseDto = GetAccountsResponseDto.AccountResponseDto
                    .builder()
                    .bankId(bank.getUuid())
    ...
    public GetLinkedAccountListResponseDto getLinkedAccounts(Long userId) {
        ...
        // user테이블 돌았을 때, 존재하지 않는 userId(userRepository.findById(userId).size() == 0) 이면
        // 404

        // 레포에서 이 사용자의 모든 계좌 가져오기
        List<Account> accountList = accountRepository.findByUser(userRepository.findById(userId)
    ...
    public GetAccountBalanceResponseDto getAccountBalance(Long userId, String accountUuid) {
        ...
        GetAccountBalanceResponseDto getAccountBalanceResponseDto = GetAccountBalanceResponseDto.builder()
                ...
                .data(GetAccountBalanceResponseDto.Data.builder().accountId(accountUuid).balance(account.getBalance())
                        .build())
                .build();
    ...
    public RequestVerificationResponseDto requestVerification(String userName, String bankUuid, String accountNo) {
        ...
        // user 조회 -> userKey 획득하기
        Email email = emailRepository.findByName(userName)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "AccountService - 000"));

        // SSAFY 금융 API >>>>> 2.2.2 사용자 계정 조회
        // 요청보낼 url
        String url1 = ssafyBaseUrl + "/ssafy/api/v1/member/search";
        ...
        Bank bank = bankRepository.findByUuid(bankUuid)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "AccountService - 002"));
        String bankName = bank.getName();

        // SSAFY 금융 API >>>>> 2.9.1 1원 송금
        // 요청보낼 url
        String url = ssafyBaseUrl + "/ssafy/api/v1/edu/accountAuth/openAccountAuth";
        Map<String, String> formattedDateTime = LocalDateTimeFormatter.formatter();
        Header header = Header.builder()
        ...
        // 요청보낼 url
        String url2 = ssafyBaseUrl + "/ssafy/api/v1/edu/demandDeposit/inquireTransactionHistory";
        ...
        Header header2 = Header.builder()
                ...
                .institutionTransactionUniqueNo(formattedDateTime2.get("date") + formattedDateTime2.get("time")
                        + RandomNumberGenerator.generateRandomNumber())
                .apiKey(ssafyFinanceApiKey)
                .userKey(userKey)
                .build();
        ...
        RequestVerificationResponseDto requestVerificationResponseDto = RequestVerificationResponseDto.builder()
                ...
                .data(RequestVerificationResponseDto.Data.builder()
                        .authIdentifier(httpResponse2.getBody().getREC().getTransactionSummary()).build())
                .build();
    ...
    public VerifyAccountResponseDto verifyAccount(String userName, String accountNo, String authText, String authCode) {
        ...
        // 요청보낼 url
        String url = ssafyBaseUrl + "/ssafy/api/v1/edu/accountAuth/checkAuthCode";
        Map<String, String> formattedDateTime = LocalDateTimeFormatter.formatter();
        Header header = Header.builder()
    ...
    public AddAccountResponseDto addAccount(long userId, List<AddAccountRequestDto.AccountDto> accounts) {
        ...
        for (AddAccountRequestDto.AccountDto accountDto : accounts) {
            ...
            // user 조회 -> userKey 획득하기
            User user = userRepository.findById(userId)
                    .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "AccountService - 005"));
            String userKey = user.getUserKey();
            ...
            // 요청보낼 url
            String url2 = ssafyBaseUrl + "/ssafy/api/v1/edu/demandDeposit/inquireDemandDepositAccountBalance";
            Map<String, String> formattedDateTime2 = LocalDateTimeFormatter.formatter();
            Header header2 = Header.builder()
            ...
            Bank bank = bankRepository.findByUuid(accountDto.getBankId())
                    .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "AccountService - 002"));
