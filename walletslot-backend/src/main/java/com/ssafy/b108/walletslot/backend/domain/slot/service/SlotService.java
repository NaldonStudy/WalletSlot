...
public class SlotService {
    ...
    // 5-1-2
    public ModifyAccountSlotResponseDto modifyAccountSlot(Long userId, String accountUuid, String accountSlotUuid,
            ModifyAccountSlotRequestDto request) {

        // User 조회
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_FOUND, "SlotService - 000"));

        // userId != account userId 이면 403 응답
        Account account = accountRepository.findByUuid(accountUuid)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "[SlotService - 002]"));
        ...
        AccountSlot accountSlot = accountSlotRepository.findByUuid(accountSlotUuid)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "[SlotService - 005]"));
        ...
        if (request.getNewBudget() != null) {
            if (request.getNewBudget() == 0) {
                ...
                AccountSlot uncategorizedAccountSlot = accountSlotRepository
                        .findUncategorizedAccountSlot(accountSlotUuid)
                        .orElseThrow(() -> new AppException(ErrorCode.MISSING_UNCATEGORIZED_SLOT, "SlotService - 000"));
                uncategorizedAccountSlot
                        .updateBudget(uncategorizedAccountSlot.getCurrentBudget() + accountSlot.getCurrentBudget());
                ...
                if (accountSlot.getSpent() > accountSlot.getCurrentBudget()) {
                    if (accountSlot.isBudgetExceeded() == true) {
                        ...
                        PushEndpoint pushEndpoint = pushEndpointRepository.findByUser(user)
                                .orElseThrow(() -> new AppException(ErrorCode.PUSH_ENDPOINT_NOTFOUND,
                                        "푸시알림을 받을 기기가 없는 사용자입니다. 기기를 등록해주세요."));
                        if (pushEndpoint.getStatus() == PushEndpoint.Status.ACTIVE) {
                            fcmService
                                    .sendMessage(pushEndpoint.getToken(), notification.getTitle(),
        ...
        ModifyAccountSlotResponseDto modifyAccountSlotResponseDto = ModifyAccountSlotResponseDto.builder()
                ...
                .data(ModifyAccountSlotResponseDto.Data.builder().accountSlotId(accountSlot.getUuid())
                        .customName(request.getCustomName()).newBudget(request.getNewBudget()).build())
                .build();
    ...
    public RemoveAccountSlotResponseDto removeAccountSlot(Long userId, String accountUuid, String accountSlotUuid) {
        ...
        // userId != account userId 이면 403 응답
        Account account = accountRepository.findByUuid(accountUuid)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "SlotService - 007"));
        if (userId != account.getUser().getId()) {
        ...
        // accountSlot 조회
        AccountSlot accountSlot = accountSlotRepository.findByUuid(accountSlotUuid)
                .orElseThrow(() -> new AppException(ErrorCode.ACCOUNT_SLOT_NOT_FOUND, "SlotService - 009"));
    ...
    public GetAccountSlotListResponseDto getAccountSlotList(Long userId, String accountUuid) {
        ...
        Account account = accountRepository.findByUuid(accountUuid)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "[SlotService - 012]"));
        if (userId != account.getUser().getId()) {
    ...
    public GetSlotHistoryResponseDto getSlotHistory(Long userId, String accountUuid, String accountSlotUuid) {
        ...
        // userId != account userId 이면 403 응답
        Account account = accountRepository.findByUuid(accountUuid)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "[SlotService - 016]"));
        if (userId != account.getUser().getId()) {
        ...
        // AccountSlot 객체 조회
        AccountSlot accountSlot = accountSlotRepository.findByUuid(accountSlotUuid)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "[SlotService - 019]"));
    ...
    // 5-2-1
    public RecommendSlotsResponseDto recommendSlots(Long userId, String accountUuid, String startDateStr,
            String endDateStr) {

        // userId != account userId 이면 403 응답
        Account account = accountRepository.findByUuid(accountUuid)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "[SlotService - 021]"));
        if (userId != account.getUser().getId()) {
        ...
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "[SlotService - 023]"));
        String userKey = user.getUserKey();

        // SSAFY 금융 API >>>>> 2.4.12 계좌 거래 내역 조회
        // 요청보낼 url
        String url1 = ssafyBaseUrl + "/ssafy/api/v1/edu/demandDeposit/inquireTransactionHistoryList";
        ...
        Header header = Header.builder()
                ...
                .institutionTransactionUniqueNo(formattedDateTime.get("date") + formattedDateTime.get("time")
        ...
        // 요청보낼 url
        String url3 = ssafyBaseUrl + "/ssafy/api/v1/edu/demandDeposit/inquireDemandDepositAccountBalance";
        ...
        Header header3 = Header.builder()
                ...
                .institutionTransactionUniqueNo(formattedDateTime3.get("date") + formattedDateTime3.get("time")
                        + RandomNumberGenerator.generateRandomNumber())
                .apiKey(ssafyFinanceApiKey)
                .userKey(userKey)
                .build();
        ...
        RecommendSlotsResponseDto recommendSlotsResponseDto = RecommendSlotsResponseDto.builder()
                ...
                .data(RecommendSlotsResponseDto.Data.builder().bank(bankDto).account(accountDto)
                        .recommededSlots(recommenededSlotDtos).build())
                .build();
    ...
    // 5-2-1
    public RecommendSlotsByProfileResponseDto recommendSlotsByProfile(Long userId, String accountUuid,
            RecommendSlotsByProfileRequestDto request) {
        ...
        // dto 조립
        RecommendSlotsByProfileResponseDto recommendSlotsByProfileResponseDto = RecommendSlotsByProfileResponseDto
                .builder()
                .success(true)
                .message("[SlotService - 028] 슬롯 추천 성공")
                .data(RecommendSlotsByProfileResponseDto.Data.builder().bank(bankDto).account(accountDto)
                        .recommededSlots(recommendedSlotDtos).build())
                .build();
    ...
    // 5-2-2
    public AddSlotListResponseDto addSlots(Long userId, String accountUuid,
            List<AddSlotListRequestDto.SlotDto> requestSlotDtos) {

        // user 조회 (없으면 404)
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_FOUND, "SlotService - 029"));

        // account 조회 (없으면 404)
        Account account = accountRepository.findByUuid(accountUuid)
                .orElseThrow(() -> new AppException(ErrorCode.ACCOUNT_NOT_FOUND, "SlotService - 030"));
        ...
        // 추가하려는 슬롯의 initialBudget합을 저장할 변수와 응답할 때 쓸 SlotDtoList 미리 만들어두기 (initialBudget
        // 누적합이 기존 미분류 슬롯 잔액을 넘으면 안됨)
        ...
        for (AddSlotListRequestDto.SlotDto requestSlotDto : requestSlotDtos) {
            ...
            // slot 조회 (없으면 404)
            Slot slot = slotRepository.findByUuid(requestSlotDto.getSlotId())
                    .orElseThrow(() -> new AppException(ErrorCode.SLOT_NOT_FOUND, "[SlotService - 033]"));
        ...
        // 일단 미분류 슬롯 찾기
        Slot uncategorizedSlot = slotRepository.findById(0L)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "SlotService - 034"));
        AccountSlot uncategorizedAccountSlot = accountSlotRepository.findByAccountAndSlot(account, uncategorizedSlot)
                .orElseThrow(() -> new AppException(ErrorCode.MISSING_UNCATEGORIZED_SLOT, "SlotService - 035"));
    ...
    // 5-2-3
    public ModifyAccountSlotListResponseDto modifyAccountSlots(Long userId, String accountUuid,
            List<ModifyAccountSlotListRequestDto.SlotDto> slotDtos) {

        // userId != account userId이면 403
        Account account = accountRepository.findByUuid(accountUuid)
                .orElseThrow(() -> new AppException(ErrorCode.NOT_FOUND, "[SlotService - 036]"));
        if (userId != account.getUser().getId()) {
        ...
        if (accountsSlots.size() == 0) { // 최초 슬롯리스트 등록이라면...
            ...
            for (ModifyAccountSlotListRequestDto.SlotDto slotDto : slotDtos) {
                ...
                Slot slot = slotRepository.findByUuid(slotDto.getSlotId())
                        .orElseThrow(() -> new AppException(ErrorCode.SLOT_NOT_FOUND, "SlotService - 037"));
                ...
                // 응답 DTO
                ModifyAccountSlotListResponseDto.SlotDto responseSlotDto = ModifyAccountSlotListResponseDto.SlotDto
                        .builder()
                        .accountSlotId(accountSlot.getUuid())
                        .name(slot.getName())
                        .isSaving(slot.isSaving())
            ...
            for (ModifyAccountSlotListRequestDto.SlotDto slotDto : slotDtos) {
                Slot slot = slotRepository.findByUuid(slotDto.getSlotId())
                        .orElseThrow(() -> new AppException(ErrorCode.SLOT_NOT_FOUND, "SlotService - 037"));
                if (existingSlotUuids.contains(slot.getUuid())) { // 만약 기존에 있던 슬롯인데 기존의 currentBudget보다 예산을 늘린다면 예산을
                                                                  // 늘린다고 판단하고 추가예산 총합계산
                    AccountSlot existingAccountSlot = accountSlotRepository.findByAccountAndSlot(account, slot)
                            .orElseThrow(() -> new AppException(ErrorCode.ACCOUNT_SLOT_NOT_FOUND, "SlotService - 037"));
                    if (slotDto.getInitialBudget() > existingAccountSlot.getCurrentBudget()) { // 이러면 예산 늘리려는 슬롯인 것
                        increaseBudgetAmount += (slotDto.getInitialBudget() - existingAccountSlot.getCurrentBudget()); // 늘리려는
                                                                                                                       // 예산
                                                                                                                       // 총합에
                                                                                                                       // 누적
                    }
